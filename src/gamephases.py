...
class AnnouncePhase(State):
    ...
    def __init__(self, next_phase):
        ...
        common.blit(
            ...
            divide(
                subtract(common.screen.get_size(), self.announce_surface.get_size()), 2
            ),
        )
        common.update()
    ...
    events = {KEYDOWN: proceed_if_waited_long_enough}
    ...
    def cleanup(self):
        def clean_walls():
            ...
            for player in game.players:
                ...
                mask = logical_and(
                    where(adj_walls == 0, 1, 0), where(field == player_id, 1, 0)
                )
                putmask(field, mask, (Field.EMPTY,))

                # delete every fourth wall with only one adjacent wall
                mask = logical_and(
                    where(adj_walls == 1, 1, 0), where(field == player_id, 1, 0)
                )
...
class PlacePhase(Phase):
    ...
    def phase_end(self):
        return self.time_left <= 0 or not [
            player for player in game.players if player.place_player.new_cannons > 0
        ]
...
class BattlePhase(Phase):
    ...
    def __init__(self):
        ...
        remove_garbage()
        self.announce_surface = common.font.render(
            "Shoot at your enemy's walls!", True, (0, 0, 0)
        )
        Phase.__init__(self)
...
class SelectPhase(Phase):
    ...
    def phase_end(self):
        return self.time_left <= 0 or not [
            player for player in game.players if not player.select_player.finished
        ]
