...
class Widget(object):
    ...
    def __init__(self, pos, text=None, surfaces=None, on_click=None):
        ...
        for i in (0, 1):
            if pos[i] == None:
                pos[i] = (
                    common.screen.get_size()[i] - self.normal_surface.get_size()[i]
                ) / 2
            elif pos[i] < 0:
                pos[i] = (
                    common.screen.get_size()[i]
                    + pos[i]
                    - self.normal_surface.get_size()[i]
                )
        self.pos = pos
...
class SpinBox(Widget):
    ...
    def __init__(self, label, pos, choices, default=None, on_change=None):
        ...
        down_pos = (
            ...
            self.pos[1]
            + self.normal_surface.get_height()
            - SpinBox.down[0].get_height(),
        )
        self.up_widget = Widget(
            up_pos, surfaces=SpinBox.up, on_click=lambda: self.change(-1)
        )
        self.down_widget = Widget(
            down_pos, surfaces=SpinBox.down, on_click=lambda: self.change(1)
        )
