...
class Map(pb.Copyable, pb.RemoteCopy):
    ...
    def apply(self):
        ...
        reactor.callLater(0, lambda: game.field.add_houses(self.houses))
        self.add_end_test()
...
class GruntAssault(Map):
    ...
    def __init__(self, options):
        ...
        self.difficulty_number = dict(
            very_easy=-20, easy=-10, medium=0, hard=10, very_hard=20
        )[self.difficulty]
    ...
    def setup(self):
        ...
        self.castles = [
            Castle(
                pos=(3 + randint(0, 7) + i * 11, randint(3, 25)), player=None, map=self
            )
            for i in (0, 1, 2)
        ]
...
class RiverMap2p(RiverMap):
    ...
    def setup(self):
        ...
        for i in (0, 1, 2):
            self.castles += [
                Castle(
                    pos=(8 + randint(-2, 2), (i + 1) * 8 - 2 + randint(-1, 1)),
                    player=game.players[0],
                    map=self,
                ),
                Castle(
                    pos=(30 + randint(-2, 2), (i + 1) * 8 - 2 + randint(-1, 1)),
...
class RiverMap4p(RiverMap):
    ...
    def setup(self):
        ...
        for i in (0, 1):
            self.castles += [
                Castle(
                    (5 + randint(-1, 1) + i * 8, 6 + randint(-1, 1)),
                    player=game.players[0],
                    map=self,
                ),
                Castle(
                    (25 + randint(-1, 1) + i * 8, 6 + randint(-1, 1)),
                    player=game.players[1],
                    map=self,
                ),
            ]
            if game.number_of_players >= 3:
                self.castles += [
                    Castle(
                        (5 + randint(-1, 1) + i * 8, 21 + randint(-1, 1)),
            ...
            if game.number_of_players >= 4:
                self.castles += [
                    Castle(
                        (25 + randint(-1, 1) + i * 8, 21 + randint(-1, 1)),
