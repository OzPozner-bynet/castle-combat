...
class BuildPlayer(BasePlayer):
    ...
    def handle_event(self, event):
        self.handle_build_or_place_event(
            event, (self.put_block, self.rotate_block), self.pos
        )
            ...
            for x in range(3):
                for y in range(3):
                    if self.block[x][y]:
                        ...
                        ypos = y + self.pos[1] - 1
                        screen_pos = (
                            xpos * common.block_size,
                            ypos * common.block_size,
                        )
        ...
        for x in range(3):
            for y in range(3):
                if self.block[x][y]:
                    ...
                    ypos = y + self.pos[1] - 1
                    if (
                        "field_owners" in game.field.map.game_options
                        and field.owner[xpos][ypos] != self.player.player_id
                    ):
                ...
                if self.block[x][y]:
                    ...
                    if field[xpos][ypos] == Field.HOUSE:
                        ...
                        common.backbuffer_blit(
                            self.player.wall_pic,
                            (xpos * common.block_size, ypos * common.block_size),
                        )
    ...
    def bounded_pos(self):
        ...
        return tuple(
            common.bound(
                self.pos[dim],
                1 - margin[dim][0],
                common.field_size[dim] - 2 + margin[dim][1],
            )
            for dim in (0, 1)
        )
    ...
    def rotate_block(self):
        if not self.block:
            raise common.ActionNotPossible
        self.block = (
            (self.block[0][2], self.block[1][2], self.block[2][2]),
    ...
    def generate_block(self, piece, turn):
        if piece in (0, 1):
            self.block = ((0, 1, 0), (0, 1, 0), (0, 1, 1))
        elif piece == 2:
            self.block = ((0, 1, 0), (0, 1, 0), (1, 1, 0))
        elif piece in (3, 4, 5):
            self.block = ((0, 1, 0), (1, 1, 0), (0, 0, 0))
        elif piece in (6, 7):
            self.block = ((0, 1, 0), (0, 1, 0), (0, 1, 0))
        elif piece in (8, 9):
            self.block = ((1, 1, 0), (0, 1, 0), (1, 1, 0))
        elif piece == 10:
            self.block = ((0, 0, 0), (0, 1, 0), (0, 0, 0))
